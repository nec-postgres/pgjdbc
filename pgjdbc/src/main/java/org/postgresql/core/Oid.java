 ...
 * Portions Copyright (c) 2015-2018, NEC Corporation
...
public class Oid {
  ...
  public static int ENCRYPT_INTEGER = 0;
  ...
  public static int valueOf(String oid) throws PSQLException {
    ...
    /**
     * Add OIDs of encrypt_text, encrypt_bytea, encrypt_numeric, encrypt_timestamp, encrypt_integer 
     * to variables.
     * @param baseCon It's used to execute SQL internally.
     * @throws SQLException
     */
    public static void setEncryptTypeOid(BaseConnection baseCon) throws SQLException{
        String sql = "SELECT typname, oid FROM pg_type " +
                     "WHERE typname in ('encrypt_text','encrypt_bytea','encrypt_numeric','encrypt_timestamp','encrypt_integer')";

        java.sql.Statement stmt = null;
        java.sql.ResultSet rs = null;
        try {
            stmt = baseCon.createStatement();
            try {
                ...
                while (rs.next()) {
                    ...
                    case "encrypt_timestamp" :
                        ...
                        break;
                    case "encrypt_integer" :
                        ENCRYPT_INTEGER = rs.getInt("oid");
                        break;
                    default :
                        break;
                    }
                    ...
                    if (LOGGER.isLoggable(Level.FINEST)) {
                        LOGGER.log(Level.FINEST,"TDEforPG: set Oid for {0}",rs.getString("typname"));
                    }
                }
            } finally {
                if (rs != null) {
